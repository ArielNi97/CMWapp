...
def run_load_ml_models():
    
    Pn = joblib.load('models/CATB_Pn (kg)_model.pkl')
    Mn = joblib.load('models/CATB_Mn (kg-m)_model.pkl')
    V = joblib.load('models/CATB_V (kg)_model.pkl')
    Vcr = joblib.load('models/CATB_Vcr (kg)_model.pkl')
    Vm = joblib.load('models/CATB_Vm (kg)_model.pkl')
    Vu = joblib.load('models/CATB_Vu (kg)_model.pkl')
    M3 = joblib.load('models/CATB_M3_0.33Pn (kg-m)_model.pkl')
    M2 = joblib.load('models/CATB_M2_0.33Pn (kg-m)_model.pkl')
    Driftu = joblib.load('models/CATB_Drift_u (-)_model.pkl')
    Driftm = joblib.load('models/CATB_Drift_m (-)_model.pkl')
    Driftcr = joblib.load('models/CATB_Drift_cr (-)_model.pkl')
        

    # Reading the user input

    UserInput = pd.read_csv('Inputs/InputForML.csv')
    Geometry = pd.read_csv('Inputs/Input_Walls_geometry.csv')
    ...
    Pn_final.to_csv('outputs/Pn.csv', index = False)   
    Mn_final.to_csv('outputs/Mn.csv', index = False) 
    V_final.to_csv('outputs/IPVn.csv', index = False) 

    BB0_final.to_csv('outputs/BBC0.csv', index = False) 
    BB1_final.to_csv('outputs/BBC1.csv', index = False) 
    BB2_final.to_csv('outputs/BBC2.csv', index = False) 
    BB3_final.to_csv('outputs/BBC3.csv', index = False) 

    AB1_final.to_csv('outputs/AB1.csv', index = False) 
    AB2_final.to_csv('outputs/AB2.csv', index = False) 
    AB3_final.to_csv('outputs/AB3.csv', index = False) 
    AB4_final.to_csv('outputs/AB4.csv', index = False) 
